...
export default class Question {
  constructor(question, qindex) {
    ...
    let showTypeTitleInfo = [];
    let contentTypeName = this.contentType ? this.contentType.name : '';
    if (contentTypeName) {
      showTypeTitleInfo.push(contentTypeName);
    }
    showTypeTitleInfo.push(this.difficultyStr);
    let knowledgePoints = [];
    this.knowledgePoints.forEach((knowledgePoint) => {
      if (knowledgePoint.name) {
        knowledgePoints.push(knowledgePoint.name);
      }
    });
    this.knowledgePointsStr = '';
    if (knowledgePoints.length) {
      this.knowledgePointsStr = knowledgePoints.join('、');
      let knowledgePointsHtml = "<span class='knowledge-points-wrapper' title='" + this.knowledgePointsStr + "'>" + this.knowledgePointsStr + "</span>";
      // showTypeTitleInfo.push(knowledgePointsHtml);
    }
