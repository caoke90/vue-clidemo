...
import { externalConfig } from "../common/js/external";
//购物车
...
export default {
  ...
  methods: {
    ...
    init: function(res, pageData) {
      ...
      pageData.question_boxs.forEach((item, index) => {
        pageData.parts.forEach((item1, index1) => {
          item1.questions &&
            item1.questions.forEach((item2, index2) => {
              if (item.id == item2.id) {
    ...
    refreshData: async function() {
      const [resp] = await ajax.all([
        ajax.post(
          "/v1/finalReview/FinalComprehensiveDetection",
          this.paperparams
        ),
  ...
  created: async function() {
    ...
    // 加载打点
    sendLog(log.module.m_listenRead, log.op.ls_detail_load, {
      s0: "english",
      s1: "套卷"
    });
