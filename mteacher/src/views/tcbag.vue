...
<script>
  // 导航下拉框
  import tcSelectBar from "../components/tcSelectBar.vue";
  import log from "../common/js/utils/logConfig";
  import { sendLog } from "../common/js/logger";
  import Bus from "../marvel/bus";
  Bus.addModalComponent(tcSelectBar);
  ...
  export default {
    ...
    computed: {
      ...
      finishedTime: function() {
        let time = 0;
        let card_list =
          this.pageType == "paperPackage" ? this.card_group : this.question_list;
    ...
    methods: {
      ...
      refreshData: async function(status, isFilter) {
        ...
        if (hget(content, "data.message") === "ok") {
          ...
          //修改展示状态
          this.success = "ok";
          if (this.pageType === "systemPackage") {
            updateTitle(content.data.data.category.name, "", "");
          }
          ...
          content_types = hget(content, "data.data.content_types", []);
          words = hget(content, "data.data.words", []);
          knowledge_points_list = hget(
            content,
            "data.data.knowledge_points_list",
            []
          );
          ...
          // words & knowledge_points_list均为空不显示头部筛选
          if (words.length === 0 && knowledge_points_list.length === 0) {
            this.isWordPage = false;
            if (isFilter) {
              Bus.tagBar.isWordPage = true;
              return;
            }
            Bus.tagBar.isWordPage = this.isWordPage;
          }
          ...
          // 系统题包要筛选为content_types, words字段每个item添加select字段
          if ( !this.selectBarLock) {
            ...
            knowledge_points_list.map(item => {
              item.select = false;
            });
      ...
      filterData: function(data) {
        this.selectBarLock = true;
        this.bagparams.page = 1;
        this.bagparams.word_ids = data.pointList;
        this.bagparams.content_types = data.typeList;
        this.bagparams.is_filter = data.isFilter ? "filter" : "nofilter";
        this.refreshData("select", data.isFilter);
      },
      ...
      setPackagePage: function(params) {
        ...
        this.pageUrl = "/v1/assign/loadPackageContent";
      },
    ...
    created: async function() {
      ...
      vm.isWordPage = params.is_word_package == 1 ;
      Bus.tagBar.isWordPage = vm.isWordPage;
      ...
      Bus.$on("transferSelectData", data => {
