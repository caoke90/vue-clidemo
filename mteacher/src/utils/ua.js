...
module.exports = (function () {
  ...
  for (const s in hash) {
    for (let i = 0; i < hash[s].length; i += 1) {
  ...
  detecter.osVersion = '';
  if (osVersion[detecter.os]) {
    detecter.osVersion = osVersion[detecter.os]();
    if (detecter.osVersion === u) {
      detecter.osVersion = '';
    }
  }
  /* 浏览器版本信息 */
  ...
  detecter.version = '';
  if (version[detecter.browser]) {
    detecter.version = version[detecter.browser]();
    if (detecter.version === u) {
      detecter.version = '';
    }
  }
  /* 修正*/
  if (detecter.browser === 'Edge') {
    detecter.engine = 'EdgeHTML';
  } else if (detecter.browser === 'Chrome' && parseInt(detecter.version, 10) > 27) {
    detecter.engine = 'Blink';
  } else if (detecter.browser === 'Opera' && parseInt(detecter.version, 10) > 12) {
    detecter.engine = 'Blink';
  } else if (detecter.browser === 'Yandex') {
    detecter.engine = 'Blink';
  }
  detecter.versionCompare = function (a, b) {
