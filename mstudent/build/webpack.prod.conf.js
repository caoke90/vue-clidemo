...
const env = process.env.STAGE=='production'?require('../config/prod.env'):
            process.env.STAGE=='staging'?require('../config/staging.env'):
            process.env.STAGE=='test'?require('../config/test.env'):
...
//webpack.DefinePlugin 只能定义常量，不能定义对象
const definePlugin={};
for(let k in env){
  console.log(k)
  definePlugin['process.env.'+k]=env[k];
}
