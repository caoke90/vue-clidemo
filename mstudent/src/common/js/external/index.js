 ...
 * http://wiki.17zuoye.net/pages/viewpage.action?pageId=22741120
 */
 ...
 * @param {string} logStr
 */
export function remoteLog(tag, logStr) {
  tag = tag || '';
  logStr = logStr || '';
  if (hasExternal('log_b')) {
    doExternal('log_b', tag, logStr);
  } else {
...
export function updateTitle(title, txtColor, bgColor) {
  ...
  if (hasExternal('updateTitle')) {
    doExternal('updateTitle', title, txtColor, bgColor);
  } else {
 ...
 * @param {string} params.confirmOnQuit 退出时是否显示对话框
 * @param {string} params.confirmOnQuitMsg 对话框的msg
 */

export function externalConfig(params) {
  let _params = {
    source:params.source,    
    confirmOnQuit: params.confirmOnQuit,
    confirmOnQuitMsg: params.confirmOnQuitMsg,
    confirmLeftTxt: params.confirmLeftTxt,
    confirmRightTxt: params.confirmRightTxt,
    confirmLeftNeedCallBack: params.confirmLeftNeedCallBack,
    confirmRightNeedCallBack: params.confirmRightNeedCallBack
  };

  _params = JSON.stringify(_params);
  if (hasExternal('externalConfig')) {
    doExternal('externalConfig', _params);
  } else {
    console.log('[no external] externalConfig----->params: ' + params);
  }
}
 ...
 * 更新原生购题车数据
 * @param {Object} params
 * @param {number} params.question_count  题目数量
 * @param {number} params.duration_minutes  预计答题时长，分钟
 * @param {Object} params.cart_detail
 * @param {string} params.cart_detail.block_name  题目分类
 * @param {string} params.cart_detail.block_value 题目数量
 */
export function updateCart(params) {
  let _params = JSON.stringify(params);
  if (hasExternal('updateCart')) {
    doExternal('updateCart', _params);
  } else {
    console.log('[no external] updateCart----->params: ' + params);
  }
}
