...
import shanks from '17zy_jxt_client_plugin';
...
export function hasExternal(method) {
  ...
  shanks.check_external(method, function (exists) {
    isExists = exists;
  });
  if (!isExists) {
...
export function doExternal(method, ...args) {
  return shanks.do_external(method, ...args);
}
...
export function bindTrigger(method, cb, once) {
  ...
  shanks.bind_trigger(method, cb, once);
}
